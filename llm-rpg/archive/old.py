            ...
            if not self.following_path:
                ...
                if self.decision[0] == 'move':
                    ...
                    if not self.will_collide_at(new_x, new_y):
                        self.center_x = new_x
                        self.center_y = new_y
                        if hasattr(self, 'path') and self.path and abs(self.center_x - new_x) <= GRID_SIZE/2 and abs(self.center_y - new_y) <= GRID_SIZE/2:
                            self.path.pop(0)
                            if not self.path:
                                self.following_path = False
                    else:
                        if self.distance_to_other(target_npc) <= GRID_SIZE:  # If we're close enough to the target
                            self.following_path = False
                            self.decision = self.brain.decide()  # get a new decision
